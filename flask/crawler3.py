...
def context_crwl(inner_url):
	...
	if inner_soup.find('div', {'id' : re.compile('bookIntroContent')}) != None:
		...
		if book_content != None:
			temp_string = book_content[0:1330]
			temp_index = temp_string.rfind('.')
			data_format['book_content'] = temp_string[0:temp_index+1]
	else:
...
def main():
	...
	for i in range(0, len(genre_list)):
		...
		for url in url_list:
			p_cnt = p_cnt + 1
			result_data.append(context_crwl(url))
			p_pcnt =  p_cnt/url_cnt*100
			print("Progress : %d%%", p_pcnt)
